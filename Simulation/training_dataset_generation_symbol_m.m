...
numFrames = 8*2; % 1 frame = 10 sub-frames
...
SNRLevels = [5, 10, 15, 20]; % in dB
UEVelocity = [5, 15, 30, 45, 60]; % in km/h
...
numTrainingSamples = 100; % number of total training samples generated
folder = '<your_proj_folder_path>/Data/Training';
...
channelModels = ["CDL-A", "CDL-B", "CDL-C"];
ChannelModelsText = ['A', 'B', 'C'];
carrierFreq = 28e9; % 28 GHz

cancelFlag = false;
for nChannel = 0:numTrainingSamples-1
    if cancelFlag == true
        break
    end
    ...
    fileEnding = "training_SNR_" + SNRLevel + "_UEV_" + UEV + "_NFRA_" + numFrames +  "_DEL_" + channelModelTxt + ".mat";
