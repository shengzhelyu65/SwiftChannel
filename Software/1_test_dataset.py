...
if __name__ == '__main__':
    ...
    all_h_estimated_tests = []
    all_h_perfect_srs_tests = []
    for SNR_level in SNR_levels:
        for UEV in UEVs:
            ...
            with h5py.File(testing_folder_path + hEst_files[0], 'r') as hf:
                h_estimated_tests = hf['h_estimated_tests'][:]
            with h5py.File(testing_folder_path + hPerfect_files[0], 'r') as hf:
            ...
            h_estimated_tests = h_estimated_tests.astype(np.float32)
            h_perfect_srs_tests = h_perfect_srs_tests.astype(np.float32)
            
            h_estimated_tests = h_estimated_tests.transpose(0, 2, 3, 1)
            h_perfect_srs_tests = h_perfect_srs_tests.transpose(0, 2, 3, 1)
        
            print("Shape of h_estimated_tests: ", h_estimated_tests.shape)
            print("Shape of h_perfect_srs_tests: ", h_perfect_srs_tests.shape)
            
            estimated_binary_path = testing_folder_path + f'all_estimated_tests_{i}.bin'
            perfect_binary_path = testing_folder_path + f'all_perfect_tests_{i}.bin'
            
            with open(estimated_binary_path, 'wb') as f:
                h_estimated_tests.tofile(f)
                
            with open(perfect_binary_path, 'wb') as f:
                h_perfect_srs_tests.tofile(f)
                
            all_h_estimated_tests.append(h_estimated_tests)
            all_h_perfect_srs_tests.append(h_perfect_srs_tests)
                
            i += 1
                
    all_h_estimated_tests = np.array(all_h_estimated_tests).reshape(-1, h_estimated_tests.shape[1], h_estimated_tests.shape[2], h_estimated_tests.shape[3])
    all_h_perfect_srs_tests = np.array(all_h_perfect_srs_tests).reshape(-1, h_perfect_srs_tests.shape[1], h_perfect_srs_tests.shape[2], h_perfect_srs_tests.shape[3])
    
    print("Shape of all_h_estimated_tests: ", all_h_estimated_tests.shape)
    print("Shape of all_h_perfect_srs_tests: ", all_h_perfect_srs_tests.shape)
    
    estimated_binary_path = testing_folder_path + f'all_estimated_tests.bin'
    perfect_binary_path = testing_folder_path + f'all_perfect_tests.bin'
            
    with open(estimated_binary_path, 'wb') as f:
        all_h_estimated_tests.tofile(f)
        
    with open(perfect_binary_path, 'wb') as f:
        all_h_perfect_srs_tests.tofile(f)
            
    print("----- Converted to binary files -----")
    print("All SNR levels and UEVs Done")
